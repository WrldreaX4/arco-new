...
export class EditfinancialreportComponent implements OnInit {
  ...
  retrieveEventReport(id: number): void {
    this.http.get(`http://localhost/arco2/arco/api/financialreportonly/${id}`).subscribe(
      (resp: any) => {
        console.log(resp);
  ...
  submitAndNavigate(id: number): void {
    if (this.financialReportForm.valid) {
      ...
      this.http.post(`http://localhost/arco2/arco/api/edit_financialreport/${id}`, reportData)
        .subscribe(
          (resp) => {
